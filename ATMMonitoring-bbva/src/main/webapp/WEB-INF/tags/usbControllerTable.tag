<%@tag description="USB Controller Table" pageEncoding="UTF-8"%>
<%@attribute name="hardwareDevices" required="true" type="java.util.Set"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@taglib uri="http://www.springframework.org/tags" prefix="spring"%>

<h3 class="txt content_hide"><spring:message code="label.usbControllers"/></h3>
...
<tr>
	...
	<th><spring:message code="label.hardwareDevice.protocolSupported"/></th>
	<th><spring:message code="label.hardwareDevice.status"/></th>
</tr>
<c:forEach items="${hardwareDevices}" var="hardwareDevice">
    <tr id="HardwareDeviceUSBCT.${hardwareDevice.id}.${hardwareDevice.startDate.time}">
    	...
    	<c:if test="${hardwareDevice.protocolSupported != null}">
			<td>${hardwareDevice.protocolSupported}</td>
		</c:if>
		<c:if test="${hardwareDevice.protocolSupported == null}">
			<td/>
		</c:if>
